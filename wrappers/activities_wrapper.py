...
class ActivitiesWrapper:
    ...
    def run_activities_flow(self,city_name:str,parameters:Dict[str,str],user_query:str,category:str)->str:
        
        print("Executing Activities Search Wrapper")
        start_time=time.time()

        print("activities in city ",city_name)

        category=category

        print(f"   🔎 Filter Category: {category}")

        results=activities_tool._run(
            cityName=city_name,
            category=category,
            maxResults=50
        )
        ...
        print("Activities Wrapper : Generating response")

        system_prompt="""
        You are a helpful travel assistant specialized in Activities.
        
        IMPORTANT: You must response with a RAW JSON object ONLY. No markdown formatting, no code blocks, no explanation text.
        
        Goal: Recommend top 3 activities from the provided database results based on the user's query.

        OUTPUT FORMAT:
        {
            "recommendations": [
                {"_id": "unique_id_string", "activities": "Activity Name", "reason": "Why this matches"},
                {"_id": "unique_id_string", "activities": "Activity Name", "reason": "Why this matches"}
            ]
        }

        RULES:
        1. Each recommendation must have "_id" (as string) and "activities".
...
activities_wrapper=ActivitiesWrapper()