...
class PlaceWrapper:
    ...
    def run_place_flow(self,city_name:str,parameters:Dict[str,str],user_query:str,category:str)->str:
        
        print("Executing Place Search Wrapper")
        start_time=time.time()

        print("Places in city ",city_name)

        category=category

        print(f"   🔎 Filter Category: {category}")

        results=place_search_tool._run(
            cityName=city_name,
            category=category,
            maxResults=50
        )
        ...
        print(" Place Wrapper : Generating response")

        system_prompt="""
        You are a helpful travel assistant specialized in Place recommendation.
        ...
        {
            "recommendations": [
                {"_id": "unique_id_string", "places
": "Place Name", "reason": "Why this matches"},
                {"_id": "unique_id_string", "places
": "Place Name", "reason": "Why this matches"}
            ]
        }
        ...
        RULES:
        1. Each recommendation must have "_id" (as string) and "places".
        ...
        try:
            ...
            print(f"❌ LLM Error in place Wrapper: {e}")
